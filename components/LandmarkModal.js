...
const switchToEduChainNetwork = async () => {
  try {
    await window.ethereum.request({
      method: 'wallet_switchEthereumChain',
      params: [{ chainId: EDUCHAIN_NETWORK.chainId }],
    });
    console.log('Switched to EduChain Network successfully');
  } catch (switchError) {
    ...
    if (switchError.code === 4902) {
      try {
        ...
        console.log('EduChain Network added to wallet');
      } catch (addError) {
        console.error('Failed to add EduChain Network:', addError);
        throw new Error('Please add EduChain Network to your wallet manually');
      }
    } else {
  ...
  const handleClaim = async () => {
    ...
    try {
      debugLog('NETWORK_SWITCH_START', {
        targetChain: EDUCHAIN_NETWORK.chainId,
        targetNetwork: EDUCHAIN_NETWORK.chainName
      });
      
      await switchToEduChainNetwork();
      ...
      debugLog('NETWORK_STATUS', {
        ...
        targetMatches: network.chainId === parseInt(EDUCHAIN_NETWORK.chainId, 16)
      });
      ...
      if (error.code === 4902) {
        alert("Please add EduChain Network to your wallet");
      } else if (error.code === -32603) {
        debugLog('NETWORK_ERROR', {
          provider: window.ethereum?.isMetaMask,
          chainId: await window.ethereum?.request({ method: 'eth_chainId' })
        });
        alert("Network error. Please check your connection to EduChain Network");
      } else {
